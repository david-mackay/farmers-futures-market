        ...
        <section className="border-b border-border">
          ...
          <ul className="divide-y divide-border">
            {upcomingDeliveries.map((order, i) => {
              ...
              const runEscrow = async (action: 'fund' | 'deliver' | 'confirm' | 'contest') => {
          ...
          {loadingBids ? (
            ...
            <ul className="divide-y divide-border">
              {pendingBuyOrdersForFarmer.map((order) => (
                <li key={order.id}>
                  <Link
                    href={`/markets?crop=${order.crop_type}&date=${order.delivery_date}`}
            ...
            <ul className="divide-y divide-border flex-1">
              {displayCrops.map((crop) => {
                ...
                return (
                  <li key={crop}>
                    <Link
                      href={bestDate ? `/markets?crop=${crop}&date=${bestDate}` : `/markets?crop=${crop}`}
