...
import { sendUsdcFromEscrow } from '../solana/usdc';
...
async function releaseFundsToSeller(orderId: string, amountSmallestUnit: number): Promise<void> {
  ...
  await sendUsdcFromEscrow({ recipientWallet: seller.address, amountSmallestUnit });
  const now = new Date().toISOString();
  await db.run('UPDATE orders SET funds_released_at = $1 WHERE id = $2', [now, orderId]);
}
...
async function applyAutoRelease(order: Order): Promise<Order> {
  if (order.status !== OrderStatus.FILLED || !order.delivered_at || order.contested_at || order.funds_released_at || order.refunded_at) {
    return order;
  }
  const delivered = new Date(order.delivered_at).getTime();
  if (Date.now() - delivered < ONE_DAY_MS) return order;
  const amount = order.total_amount_usdc ?? Math.round((order.price / JMD_PER_USD) * order.quantity * 1e6);
  if (amount <= 0) return order;
  const seller = await getUserById(getSellerId(order));
  if (!seller?.address) return order;
  setImmediate(() => releaseFundsToSeller(order.id, amount).catch((err) => console.error('Auto-release failed', order.id, err)));
  return order;
}
...
export async function getOrders(filters: OrderFilters = {}): Promise<Order[]> {
  ...
  if (filters.delivery_month) { sql += ` AND LEFT(delivery_date, 7) = ${$()}`; params.push(filters.delivery_month); }
...
export async function createOrder(
  ...
  await db.run(
    ...
    [id, creatorId, data.crop_type, data.type, data.price, data.quantity, data.delivery_date, OrderStatus.OPEN]
  );
...
export async function initiateFillForPayment(userId: string, orderId: string): Promise<{ escrowAddress?: string; amountUsdc?: number; usdcMint?: string; error?: string }> {
  const order = await getOrderById(orderId);
  ...
  const amountUsdc = Math.round((order.price / JMD_PER_USD) * order.quantity * 1e6);
  ...
  if (result.changes === 0) {
    const current = await db.get('SELECT pending_fill_by, pending_fill_expires_at FROM orders WHERE id = $1', [orderId]) as { pending_fill_by: string | null; pending_fill_expires_at: string | null } | undefined;
    if (current?.pending_fill_by && current.pending_fill_expires_at && current.pending_fill_expires_at >= now) {
      return { error: 'Order is reserved by another buyer. Try again shortly.' };
    }
  ...
  try {
    ...
    await db.run('UPDATE orders SET pending_fill_by = NULL, pending_fill_expires_at = NULL WHERE id = $1', [orderId]);
    return { error: 'Escrow not configured' };
  }
}

export async function confirmFillWithPayment(userId: string, orderId: string, txSignature: string): Promise<{ order?: Order; voucher?: FutureVoucher; error?: string }> {
  const order = await getOrderById(orderId);
  if (!order) return { error: 'Order not found' };
  if (order.status !== OrderStatus.OPEN) return { error: 'Order is not open' };

  const row = await db.get('SELECT pending_fill_by, pending_fill_expires_at FROM orders WHERE id = $1', [orderId]) as { pending_fill_by: string | null; pending_fill_expires_at: string | null } | undefined;
  if (!row || row.pending_fill_by !== userId) return { error: 'No matching reservation. Please start the buy flow again.' };
  const now = new Date().toISOString();
  ...
  if (!verification.verified) {
    await db.run('UPDATE orders SET pending_fill_by = NULL, pending_fill_expires_at = NULL WHERE id = $1', [orderId]);
    return { error: 'Payment verification failed. Ensure the transaction succeeded and try again.' };
  }
  ...
  if (fillResult.changes === 0) {
    return { error: 'Order was already filled by someone else or your reservation expired. If you sent payment, contact support for a refund.' };
  }
  ...
  if (order.type === OrderType.ASK) {
    ...
    await db.run(
      ...
      [voucherId, orderId, userId, order.crop_type, order.quantity, order.delivery_date, order.price]
    );
...
export async function fillOrder(userId: string, orderId: string): Promise<{ order?: Order; voucher?: FutureVoucher; error?: string }> {
  const order = await getOrderById(orderId);
  if (!order) return { error: 'Order not found' };
  if (order.status !== OrderStatus.OPEN) return { error: 'Order is not open' };
  if (order.creator_id === userId) return { error: 'Cannot fill your own order' };
  ...
  const now = new Date().toISOString();
  const totalAmountUsdc = Math.round((order.price / JMD_PER_USD) * order.quantity * 1e6);
  const result = await db.run(
    ...
    [OrderStatus.FILLED, userId, now, totalAmountUsdc, orderId, OrderStatus.OPEN]
  );
  if (result.changes === 0) return { error: 'Order was already filled by someone else.' };
...
export async function cancelOrder(userId: string, orderId: string): Promise<{ order?: Order; error?: string }> {
  const order = await getOrderById(orderId);
  if (!order) return { error: 'Order not found' };
  if (order.status !== OrderStatus.OPEN) return { error: 'Order is not open' };
  if (order.creator_id !== userId) return { error: 'Only the creator can cancel this order' };
  await db.run('UPDATE orders SET status = $1 WHERE id = $2', [OrderStatus.CANCELLED, orderId]);
  return { order: (await getOrderById(orderId))! };
}

export async function initiateEscrow(userId: string, orderId: string): Promise<{ escrowAddress?: string; amountUsdc?: number; usdcMint?: string; error?: string }> {
  const order = await getOrderById(orderId);
  if (!order) return { error: 'Order not found' };
  ...
  const amountUsdc = order.total_amount_usdc ?? Math.round((order.price / JMD_PER_USD) * order.quantity * 1e6);
  if (amountUsdc <= 0) return { error: 'Invalid order amount' };
  try {
    const escrow = getEscrowKeypair();
    return { escrowAddress: escrow.publicKey.toBase58(), amountUsdc, usdcMint: getUsdcMint() };
  } catch (e) {
    return { error: 'Escrow not configured' };
  }
}

export async function fundEscrow(userId: string, orderId: string, txSignature?: string): Promise<{ order?: Order; error?: string }> {
  const order = await getOrderById(orderId);
  if (!order) return { error: 'Order not found' };
  if (order.status !== OrderStatus.FILLED) return { error: 'Order is not filled' };
  if (getBuyerId(order) !== userId) return { error: 'Only the buyer can fund escrow' };
  ...
  if (!txSignature && process.env.SKIP_ESCROW_VERIFICATION === '1') {
    const now = new Date().toISOString();
    await db.run('UPDATE orders SET escrow_funded_at = $1 WHERE id = $2', [now, orderId]);
    return { order: (await getOrderById(orderId))! };
  }
  ...
  if (!verification.verified) return { error: 'Transaction verification failed' };
  const now = new Date().toISOString();
  await db.run('UPDATE orders SET escrow_funded_at = $1 WHERE id = $2', [now, orderId]);
  return { order: (await getOrderById(orderId))! };
}

export async function attestDelivery(userId: string, orderId: string): Promise<{ order?: Order; error?: string }> {
  const order = await getOrderById(orderId);
  if (!order) return { error: 'Order not found' };
  if (order.status !== OrderStatus.FILLED) return { error: 'Order is not filled' };
  ...
  const now = new Date().toISOString();
  await db.run('UPDATE orders SET delivered_at = $1 WHERE id = $2', [now, orderId]);
  return { order: (await getOrderById(orderId))! };
}

export async function confirmReceipt(userId: string, orderId: string): Promise<{ order?: Order; error?: string }> {
  const order = await getOrderById(orderId);
  if (!order) return { error: 'Order not found' };
  if (order.status !== OrderStatus.FILLED) return { error: 'Order is not filled' };
...
export async function contestDelivery(userId: string, orderId: string): Promise<{ order?: Order; error?: string }> {
  const order = await getOrderById(orderId);
  if (!order) return { error: 'Order not found' };
  if (order.status !== OrderStatus.FILLED) return { error: 'Order is not filled' };
  ...
  const now = new Date().toISOString();
  await db.run('UPDATE orders SET contested_at = $1 WHERE id = $2', [now, orderId]);
  return { order: (await getOrderById(orderId))! };
}

export async function resolveDispute(orderId: string, resolution: 'release' | 'refund'): Promise<{ order?: Order; error?: string }> {
  const order = await getOrderById(orderId);
  if (!order) return { error: 'Order not found' };
  if (order.status !== OrderStatus.FILLED) return { error: 'Order is not filled' };
  ...
  try {
    ...
    if (!buyer?.address) return { error: 'Buyer wallet address not found' };
    await sendUsdcFromEscrow({ recipientWallet: buyer.address, amountSmallestUnit: amount });
    const now = new Date().toISOString();
    await db.run('UPDATE orders SET refunded_at = $1 WHERE id = $2', [now, orderId]);
    return { order: (await getOrderById(orderId))! };
  } catch (e) {
    console.error('resolveDispute failed', e);
    return { error: resolution === 'release' ? 'Failed to release funds to seller' : 'Failed to refund buyer' };
  }
}

export async function getVoucherById(id: string): Promise<FutureVoucher | undefined> {
  const row = await db.get('SELECT * FROM vouchers WHERE id = $1', [id]);
  return row ? rowToVoucher(row as Record<string, unknown>) : undefined;
}

export async function getVouchersByOwner(ownerId: string): Promise<FutureVoucher[]> {
  const rows = (await db.all('SELECT * FROM vouchers WHERE owner_id = $1 ORDER BY created_at DESC', [ownerId])) as Record<string, unknown>[];
  return rows.map(rowToVoucher);
}

export async function listVoucher(ownerId: string, voucherId: string, listedPrice: number): Promise<{ voucher?: FutureVoucher; error?: string }> {
  ...
  await db.run('UPDATE vouchers SET is_listed = 1, listed_price = $1 WHERE id = $2', [listedPrice, voucherId]);
  return { voucher: (await getVoucherById(voucherId))! };
}

export async function buyVoucher(buyerId: string, voucherId: string): Promise<{ voucher?: FutureVoucher; error?: string }> {
  const voucher = await getVoucherById(voucherId);
  if (!voucher) return { error: 'Voucher not found' };
  ...
  await db.run('UPDATE vouchers SET owner_id = $1, is_listed = 0, purchase_price = $2, listed_price = NULL WHERE id = $3', [buyerId, voucher.listed_price, voucherId]);
  return { voucher: (await getVoucherById(voucherId))! };
}

export async function getListedVouchers(): Promise<FutureVoucher[]> {
  const rows = (await db.all('SELECT * FROM vouchers WHERE is_listed = 1 ORDER BY created_at DESC')) as Record<string, unknown>[];
  return rows.map(rowToVoucher);
}
